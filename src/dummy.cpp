...
//                 float dist_rover_to_waypoint = sqrtf(dx * dx + dy * dy); // Distancia al waypoint
//                 float min_dist_sensor = fminf(sens.us_left_dist, fminf(sens.us_mid_dist, sens.us_right_dist)); // Distancia mínima del sensor
//                 Serial.printf("Distancia al waypoint: %.2f, Distancia mínima del sensor: %.2f\n", dist_rover_to_waypoint, min_dist_sensor);
 
//                 if (fabsf(dist_rover_to_waypoint - min_dist_sensor/50) < 0.1 && dist_rover_to_waypoint <=0.4f) {
//                     // Waypoint tapado por obstáculo
//                     Serial.println("Waypoint tapado por obstáculo, se pasa al siguiente waypoint");
//                     // Se completa el waypoint actual y se pasa al siguiente
//                     complete_current_waypoint(os);
//                     bool ok = set_waypoint(ctx_ptr); // Checkeo ante error y limpiar flag de waypoint alcanzado
//                     if (!ok) { // Si no se pudo completar el waypoint, se vuelve a STAND_BY
//                         enter_stand_by(ctx_ptr);
//                         os.state = OS_State::STAND_BY;
//                         set_operation_log(OS_State::STAND_BY, OS_State::MOVE, ctx_ptr);
...
//                         os.state = OS_State::ALIGN;
//                         set_operation_log(OS_State::ALIGN, OS_State::MOVE, ctx_ptr);
//                         Serial.printf("Waypoint completado, se pasa al siguiente waypoint: %.2f, %.2f\n", ctrl.x_d, ctrl.y_d);
